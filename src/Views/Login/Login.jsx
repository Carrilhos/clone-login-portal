...
const Login = () => {
  ...
  const handleLogin = (e) => {
      if (isOnline) {
        ...
        axios
          .post(
            `${process.env.REACT_APP_API}/autenticacao/autenticar`,
            {
              username: user,
              password,
            },
        ...
        toast.warning("Sem serviço de rede, utilizando modo offline");
        navigate(`/home/${user}`);
      }
    };
  ...
  useEffect(() => {
    const onlineStatus = () =>{navigator.onLine ? setIsOnline(true) : setIsOnline(false)}
